(function (root, factory) {
  ...
  function getFirebase(firebaseLike) {
    const fb = firebaseLike || globalThis.firebase;
  ...
  function payload(result, fb, uid) {
    ...
    if (String(result.sessionType || "diagnostic") !== "diagnostic") throw new Error("Session type diagnostic tidak valid.");
  ...
  function createPersistence(options) {
    ...
    async function saveResult(result) {
      ...
      return { resultId: data.resultId, ownerUid: user.uid, trustStatus: "client-untrusted" };
    }
    ...
    async function getResult(resultId) {
      ...
      if (String(data.ownerUid) !== String(user.uid)) throw new Error("Diagnostic result bukan milik pengguna aktif.");
  ...
  return Object.freeze({ COLLECTION, ALLOWED_ROLES, createPersistence, payload });
});
